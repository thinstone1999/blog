...
const TrafficManagementPage: React.FC = () => {
  ...
  const exportData = () => {
    ...
    trafficData.forEach((item) => {
      // 获取类别名称
      const categoryName = categories.find((cat) => cat.id === item.category)?.name || item.category
  ...
  const importData = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // 更新类别（如果需要）
        const newCategoriesList = Object.entries(newCategoriesMap).map(([name, id]) => ({
          id,
          name
        }))
    ...
    const fetchData = async () => {
      try {
        ...
        if (trafficResult.code === 0 && trafficResult.data) {
          ...
          const formattedTrafficData = trafficResult.data.map((item) => ({
            id: item.id,
        ...
        if (categoryResult.code === 0 && categoryResult.data) {
          // 将数据库返回的数据格式转换为页面使用的格式
          const formattedCategories = categoryResult.data.map((item) => ({
            id: item.id,
            name: item.name
          }))
  ...
  const handleAddTraffic = async () => {
    if (!formData.category || !formData.amount || !formData.year || !formData.month) {
      toast.error('请填写完整信息')
      return
    }
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {currentData.map((item) => {
                        const category =
                          categories.find((cat) => cat.id === item.category)?.name || '未知类别'
                        return (
